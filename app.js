...
var emailpass = process.env.PASSWORD;
...
var mailtext1 =
  ...
  // +
  "<div style='background: linear-gradient(#4286f4, #373B44); padding:20px;border-radius:5px; text-align:center; width:65vw; margin:auto; color:white'>" +
  "<span style='color:white; font-size:1.4em; margin-right:-3px; text-decoration:overline white'>add</span><img src ='https://cdn.pixabay.com/photo/2016/02/05/15/12/two-1181082_960_720.png' style='width:2.9em; height:2.9em; margin-top:-10px;'><span style='margin-left:-6px; color:white; font-size:2em; font-weight:600;'>kart</span>" +
...
var mailtext2 =
  "</h3><p>To complete your sign up, please verify your email :</p>" +
  "<div class='button' style='margin:10px auto;text-align:center;background:orangered;padding-top:5px;padding-bottom:4px;width:45vw;max-width:500px;border-radius:5px; color:white'><a style='text-decoration:none;font-weight:700;margin:auto; color:white;' href ='" +
  HOMELINK +
  "/verify/";
var mailtext3 =
  "'>Click HERE</a></div>" +
  "<p>Or copy this link and paste in your web browser</p><a style='color:orangered; margin:10px auto; text-decoration:none' href ='" +
  HOMELINK +
  "/verify/";
var mailtext4 = "'>" + HOMELINK + "/verify/";
var mailtext5 =
  "</a><hr style='border: 0; height: 1px; background-image: linear-gradient(to right, #f0f0f0, #00b9ff, #59d941, #f0f0f0);'><p style='color:lightgrey'>Cheers,<p><b>The add2kart Team</p></div> <p style='color:red;'>MADE WITH ❤️ BY AYAN & SOHAM (BE,IT @JU)</p><p>(THIS IS JUST A PROJECT WORK ,PLEASE IGNORE THIS EMAIL IF WE MAILED THE WRONG PERSON.)</p>";
...
mongoose.connect(
  "mongodb+srv://add2kart:coltismyname@cluster0-zntgd.mongodb.net/test?retryWrites=true&w=majority",
  ...
  User.find({ email: email, password: password }, function (err, foundUser) {
    if (foundUser.length == 0) {
      ...
      res.render("home.ejs", {
        login: false,
        user: null,
        message: "Wrong Email-ID / Password ! contact support...",
      });
    } else {
      if (foundUser[0].verified) {
        ...
        res.render("home.ejs", {
          ...
          user: null,
          message:
            "Account verification link has been sent to your Email-ID, please verify it to proceed further.",
        });
      }
    }
  });
  ...
  User.find({ email: email }, function (err, foundUser) {
    ...
    if (err) {
      ...
      User.create(newUser, function (err, user) {
        if (err) {
          ...
          var mailOptions = {
            ...
            html:
              mailtext1 +
              "<span style='color:orangered; background:lightgrey; padding:2px; border-radius:20px;'>" +
              email +
              "</span>" +
              mailtext2 +
              email +
      ...
      res.render("home.ejs", {
        ...
        user: null,
        message:
          "Account with that Email-ID already exists, CONTACT SUPPORT!!!",
      });
    }
  });
});
...
app.get("/verify/:email", function (req, Res) {
  ...
  User.find(myquery, function (err, foundUser) {
    if (foundUser.length == 0) Res.redirect("/");
    else if (foundUser[0].verified) Res.redirect("/");
    else {
      User.updateOne(myquery, newvalues, function (err, res) {
        if (err) {
          ...
          Res.render("home.ejs", {
            ...
            user: null,
            message:
              "You are not a registered user, please visit our website and fill the 'SignUP' form.",
          });
...
app.post("/category/:row/:col", function (req, Res) {
  ...
  if (email == "null") {
    Category.find(catquery, function (err, foundCat) {
      if (foundCat.length == 0) Res.redirect("/");
      else {
  ...
  User.find(userquery, function (err, foundUser) {
    if (err) {
      ...
      if (foundUser.length == 0) {
        ...
        User.updateOne(
          userquery,
          { cart: cart, cartValue: foundUser[0].cartValue },
          function (err, Res) {
            if (err) {
              ...
              if (err) {
                ...
                var mailOptions = {
                  ...
                  html:
                    ...
                    order.amount +
                    "</p>" +
                    "<p>THANK YOU FOR SHOPPING WITH add2kart!</p>" +
                    mailtext5,
                };
  ...
  Review.create(newData, function (err, review) {
    if (err) {
      ...
      res.render("home.ejs", {
        login: false,
        user: null,
        message: 'Thank You! For being a part of add2kart :") ',
      });
    }
  });
});
